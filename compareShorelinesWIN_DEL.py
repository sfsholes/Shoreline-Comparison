...
PATH = "D://Research//05_GlobalMarsShorelines//2_Work//IntersectShorelines//ArabiaLevel_CSV"
TOL = 0.25         # Tolerance for Longitude increments (in degrees)
#Purging this, fixed it in data collection instead
...
for filename in os.listdir(PATH):
    ...
    if filename.endswith(".txt") or filename.endswith(".csv"):
        ...
        file = open(os.path.join(PATH,filename), newline='')
        reader = csv.reader(file)
        header = next(reader)       # The first line is the header
        
        data = []
        for row in reader:
            # row = [OBID, FID, ID, FID, ID, LAT, LON]
            LON = float(row[-1])
            LON_ROUND = round(LON*(1/TOL))/(1/TOL)      #Rounds to nearest TOL
            LAT = float(row[-2])
            LON_LIST.append(LON_ROUND)
            # GENERATE A LIST OF ALL TOTAL POINTS
            POINT = (LAT, LON_ROUND, file_num)
            # Removes accidental duplicate points (purge if certain about data collection as slow)
            if POINT not in TUP_LIST:
                TUP_LIST.append(POINT)
        file_num += 1
    else:
        continue
...
for item in LON_LIST_UNIQ:
    ...
    # i is the enumerate value (0,1,2...), j is the tuple (0=lat, 1=lon, 2=file)
    print(f"item:  {item}, len: {len(TUP_LIST_TEMP)}")
    ...
    if len(TUP_LIST_TEMP) == 0:        #Check for empty set
        ...
        print(f"   lat_diff: {lat_diff:0.5}")
        
        # # 1) Sort all long points in descending order
        # LIST_PTS = TUP_LIST_TEMP
        # LIST_PTS.sort(key=lambda item:item[0], reverse=True)
        # # 2) Find how many unique file pts are present
        # unique_pts = len(set(item[-1] for item in TUP_LIST_TEMP))
        # print(unique_pts)
        ...
        #         print(f"   LIST_PTS_TMP: {LIST_PTS_TMP}")
...
while j < len(FILE_NAMES_D):
    axs[0].plot(FILE_LONS_D[j], FILE_ELEVS_D[j], '.', label=str(FILE_NAMES_D[j]), markersize=0.8, color=D_COLORS[j])
    j += 1
...
while j < len(FILE_NAMES_A):
    ...
    axs[1].plot(FILE_LONS_A[j], FILE_ELEVS_A[j], '.', label=str(FILE_NAMES_A[j]), markersize=0.8, color=A_COLORS[j])
    j += 1
